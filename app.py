...
ALUM_BAR_12FT = 3650  # 12 Foot Aluminum Track (3650 mm approx)
BLADE_THICKNESS = 3  
...
if st.button("🔴 GENERATE FULL SITE LAYOUT & BAREEK MATERIAL BILL", type="primary", use_container_width=True):
    if not window_entries:
        ...
        total_alum_track_mm = 0  # <--- New Aluminum Track Counter
        ...
        all_sash_pieces = []
        all_mesh_pieces = []
        all_alum_pieces = []  # <--- For Aluminum optimization
        ...
        for win in window_entries:
            ...
            if site_track == "2 Track":
                ...
                # 2-Track me niche 2 line aluminum track chalega width ke barabar
                ...
                # 2.5-Track me bhi 2 kanch aur 1 jaali ke liye 3 line track niche chalega width ke barabar
                ...
                num_glass_sash = 3  # <--- 3 Palle poore glass ke ginega automatic!
                num_mesh_sash = 0
                
                total_frame_3t_mm += ((w * 2) + (h * 2)) * qty
                all_frame_3t_pieces.extend([int(w), int(w), int(h), int(h)] * qty)
                
                # 3-Track me niche 3 track aluminum rail chalega width ke barabar
                total_alum_track_mm += (w * 3) * qty
                all_alum_pieces.extend([int(w), int(w), int(w)] * qty)
        ...
        if error_found:
            ...
            bars_2t_frame = len(cutting_stock_1d(STANDARD_BAR, all_frame_2t_pieces, BLADE_THICKNESS)) if all_frame_2t_pieces else 0
            bars_3t_frame = len(cutting_stock_1d(STANDARD_BAR, all_frame_3t_pieces, BLADE_THICKNESS)) if all_frame_3t_pieces else 0
            bars_sash = len(cutting_stock_1d(STANDARD_BAR, all_sash_pieces, BLADE_THICKNESS)) if all_sash_pieces else 0
            bars_mesh = len(cutting_stock_1d(STANDARD_BAR, all_mesh_pieces, BLADE_THICKNESS)) if all_mesh_pieces else 0
            bars_interlock = math.ceil(total_interlock_mm / STANDARD_BAR) if total_interlock_mm > 0 else 0
            
            # Aluminum Track 12ft bar ceiling calculation
            bars_alum = len(cutting_stock_1d(ALUM_BAR_12FT, all_alum_pieces, 2)) if all_alum_pieces else 0
            
            bm1, bm2, bm3 = st.columns(3)
            with bm1:
                st.markdown("### 🪵 Profiles Required")
                if total_frame_2t_mm > 0: st.write(f"🔹 **2-Track Outer Frame:** `{total_frame_2t_mm/1000:.1f} Mtrs` ➔ 📦 **{bars_2t_frame} Pcs (5.8m)**")
                if total_frame_3t_mm > 0: st.write(f"🔸 **3-Track Outer Frame:** `{total_frame_3t_mm/1000:.1f} Mtrs` ➔ 📦 **{bars_3t_frame} Pcs (5.8m)**")
                st.write(f"🟩 **Glass Sash Profile:** `{total_sash_mm/1000:.1f} Mtrs` ➔ 📦 **{bars_sash} Pcs (5.8m)**")
                if total_mesh_sash_mm > 0: st.write(f"🕸️ **Jaali Sash Profile:** `{total_mesh_sash_mm/1000:.1f} Mtrs` ➔ 📦 **{bars_mesh} Pcs (5.8m)**")
                if total_interlock_mm > 0: st.write(f"⛓️ **Interlock Profile:** `{total_interlock_mm/1000:.1f} Mtrs` ➔ 📦 **{bars_interlock} Pcs (5.8m)**")
                # Aluminum Display
                st.write(f"⚙️ **Aluminum Sliding Track:** `{total_alum_track_mm/1000:.1f} Mtrs` ➔ 💿 **{bars_alum} Pcs (12ft)**")
            ...
            # Cutting plans
            st.write("---")
            st.header("📐 MINIMUM WASTAGE PROFILE CUTTING PLANS")
            ...
            if all_frame_2t_pieces:
                ...
                for idx, bar in enumerate(bars, 1): st.write(f"BAR #{idx} ➔ Kaato pieces: `{bar}` mm")
            ...
            if all_alum_pieces:
                st.subheader("⚙️ Aluminum Sliding Track Cutting Plan (12ft / 3650mm Bars)")
                bars = cutting_stock_1d(ALUM_BAR_12FT, all_alum_pieces, 2)
                for idx, bar in enumerate(bars, 1): st.write(f"ALUM BAR #{idx} ➔ Kaato pieces: `{bar}` mm")
